            ...
            <div key={item.name} className="relative flex flex-col">
              ...
              {item.highlight && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 z-10">
                  <span className="px-4 py-1.5 bg-amber-300 text-slate-900 text-xs font-bold rounded-full shadow-md shadow-amber-100 whitespace-nowrap">
                    Popular
                  </span>
                </div>
              )}
              ...
              <div
                className={`flex-1 rounded-2xl flex flex-col transition-all duration-300 ${
                  item.highlight
                    ? "bg-gradient-to-b from-indigo-500 to-sky-400 border-2 border-sky-200 p-8 pt-10 shadow-2xl shadow-sky-200/60"
                    : "bg-white border border-slate-200 p-7 shadow-sm hover:shadow-md hover:border-sky-300"
                }`}
              >
                ...
                {/* Plan name */}
                <h3
                  className={`text-xl font-bold mb-4 ${item.highlight ? "text-white" : "text-slate-900"}`}
                >
                  {item.name}
                </h3>
                ...
                <div className="mb-6 flex flex-col">
                  <span
                  className={`text-3xl w-full font-extrabold leading-tight ${item.highlight ? "text-white" : "text-slate-900"}`}
                >
                  {item.price}
                </span>
                  {item.period && (
                    <span
                      className={`ml-1 text-sm font-normal ${item.highlight ? "text-sky-100" : "text-slate-500"}`}
                    >
                      {item.period}
                    </span>
                  )}
                    ...
                    <li key={feature} className="flex items-start gap-3">
                      <svg
                        className={`w-4 h-4 flex-shrink-0 mt-0.5 ${
                          item.highlight ? "text-green-100" : "text-indigo-500"
                        }`}
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                ...
                <a
                  ...
                  className={`block text-center py-3 px-5 rounded-full font-semibold text-sm transition-colors ${
                    item.highlight
                      ...
                      : "bg-gradient-to-r from-indigo-500 to-sky-400 text-white hover:opacity-95"
                  }`}
                >
                  {item.cta}
                </a>
