      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5">
          {data.items.map((item) => (
            <div
              key={item.role}
              className="bg-slate-50 border border-slate-200 hover:border-sky-300 rounded-2xl p-6 transition-colors group flex flex-col justify-start"
            >
              ...
              <div className="space-y-3">
                {item.props.map((prop) => (
                    <div key={prop} className="text-slate-500 text-sm leading-snug">• {prop}</div>
                ))}
              </div>
            </div>
          ))}
