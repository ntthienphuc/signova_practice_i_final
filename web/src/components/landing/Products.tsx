      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {data.items.map((item) => (
            <div
              key={item.title}
              className="group bg-white rounded-2xl border border-gray-200 p-6 hover:shadow-xl hover:shadow-indigo-100/60 hover:border-indigo-200 transition-all duration-300 flex flex-col"
            >
              {/* Tag */}
              <span className="inline-block self-start px-3 py-1 text-xs font-semibold text-indigo-700 bg-indigo-50 rounded-full mb-5">
                {item.tag}
              </span>
